...
-- Operaciones idempotentes y transaccionales sobre pedidos.
--
-- Soporta:
--   add_item
--   set_quantity
...
create table if not exists public.commerce_order_operations (
...
create index if not exists
  commerce_order_operations_order_idx
on public.commerce_order_operations (
  company_id,
  order_id,
  created_at desc
);
...
create index if not exists
  commerce_order_operations_status_idx
on public.commerce_order_operations (
  company_id,
  status,
  created_at
...
create index if not exists
  commerce_order_operations_message_idx
on public.commerce_order_operations (
  company_id,
  message_id
)
...
drop trigger if exists
  trg_commerce_order_operations_updated_at
on public.commerce_order_operations;
...
alter table
  public.commerce_order_operations
enable row level security;


revoke all
on table public.commerce_order_operations
